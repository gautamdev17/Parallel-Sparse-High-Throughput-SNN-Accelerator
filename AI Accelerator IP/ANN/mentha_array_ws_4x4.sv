...
// Configured with INT8 signed inputs (A*, B*) and INT32 signed accumulators (C*).

module mentha_array_ws_4x4 #(
    parameter int IDX_W    = 8,     // 8-bit index
    parameter int A_VAL_W  = 8,     // 8-bit signed INT8 stationary weight A*
    parameter int B_VAL_W  = 8,     // 8-bit signed INT8 streaming activation B*
    parameter int C_VAL_W  = 32,    // 32-bit signed INT32 stream accumulator C*
    ...
    // --- B* vertical edge inputs (top -> bottom) (INT8 signed) ---
    input  logic [COLS*IDX_W-1:0]   b_edge_idx_flat,
    input  logic signed [COLS*B_VAL_W-1:0] b_edge_val_flat,

    // --- B* vertical edge outputs (exiting bottom) ---
    output logic [COLS*IDX_W-1:0]   b_out_idx_flat,
    output logic signed [COLS*B_VAL_W-1:0] b_out_val_flat,

    // --- C* horizontal edge inputs (entering from West) (INT32 signed accumulators) ---
    input  logic [ROWS*NUM_CBUF*IDX_W-1:0]       c_edge_a_idx_flat,
    input  logic [ROWS*NUM_CBUF*IDX_W-1:0]       c_edge_b_idx_flat,
    input  logic signed [ROWS*NUM_CBUF*C_VAL_W-1:0] c_edge_val_flat,
    ...
    output logic signed [ROWS*NUM_CBUF*C_VAL_W-1:0] c_out_val_flat,
    ...
    logic signed [B_VAL_W-1:0] b_wire_val [(ROWS+1)*COLS];
    ...
    logic signed [NUM_CBUF*C_VAL_W-1:0] c_wire_val   [ROWS*(COLS+1)];
    ...
    generate
        ...
        for (c = 0; c < COLS; c++) begin : g_top_b
            ...
            assign b_wire_val[0*COLS + c] = b_edge_val_flat[c*B_VAL_W +: B_VAL_W];
        end
        ...
        for (r = 0; r < ROWS; r++) begin : g_west_c
            ...
            assign c_wire_val[r*(COLS+1) + 0]   = c_edge_val_flat[r*NUM_CBUF*C_VAL_W +: NUM_CBUF*C_VAL_W];
        ...
        for (c = 0; c < COLS; c++) begin : g_bottom_b
            ...
            assign b_out_val_flat[c*B_VAL_W +: B_VAL_W] = b_wire_val[ROWS*COLS + c];
        end
        ...
        for (r = 0; r < ROWS; r++) begin : g_east_c
            ...
            assign c_out_val_flat[r*NUM_CBUF*C_VAL_W +: NUM_CBUF*C_VAL_W]   = c_wire_val[r*(COLS+1) + COLS];
