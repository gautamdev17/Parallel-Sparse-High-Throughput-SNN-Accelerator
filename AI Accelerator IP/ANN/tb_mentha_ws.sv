...
module tb_mentha_ws;
    ...
    task automatic set_b(int col, int idx, int val);
        ...
        b_edge_val_flat[col*B_VAL_W +: B_VAL_W] = val;
    endtask
    ...
    function automatic logic signed [C_VAL_W-1:0] get_east_c_val(int row, int slot);
        return c_out_val_flat[(row*NUM_CBUF + slot)*C_VAL_W +: C_VAL_W];
    endfunction
    ...
    initial begin
        ...
        $display("=== Starting Mentha WS Streaming C* Test Suite (INT8 Weights/Activations) ===");

        // Preload stationary A* (INT8 signed values):
        ...
        // Cycle 2: B*(b_idx=20, val=6) at Col 1 as C* wave reaches PE(0,1): product = (-4)*6 = -24
        @(negedge clk);
        clear_b_all();
        set_b(1, 20, 6);
        ...
        // Verify that -15 + 6 = -9 was accumulated in-flight inside C* stream!
        check_east_c("T2: In-Flight Accumulation (-15 + 6)", 0, 10, 10, -9);
